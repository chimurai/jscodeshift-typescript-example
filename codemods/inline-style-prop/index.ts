    ...
    if (styleAttribute && styleAttribute.type === 'JSXAttribute') {
      ...
      if (
        ...
        styleAttribute.value.expression.properties = convertObjectProperties(
          ...
          needsFlexDirection(styleAttribute.value.expression.properties)
        );
        return;
      }
      ...
      if (
        ...
        root.findVariableDeclarators(objectName).forEach(path => {
          if (path.value.init.type === 'ObjectExpression') {
            ...
            if (node?.type === 'ObjectProperty' && node.value.type === 'ObjectExpression') {
              node.value.properties = convertObjectProperties(
                file.path,
                j,
                node.value.properties,
                needsFlexDirection(node.value.properties)
              );
            }
          }
        });
        ...
        root.findVariableDeclarators(objectName).forEach(path => {
          ...
          if (path.value.init.type === 'ObjectExpression') {
            ...
            node.properties = convertObjectProperties(
              ...
              j,
              node.properties,
              needsFlexDirection(node.properties)
            );
          }
        });
...
const needsFlexDirection = (
  properties: (Property | ObjectProperty | SpreadElement | SpreadProperty | ObjectMethod)[]
) => {
